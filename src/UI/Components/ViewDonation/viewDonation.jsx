...
const DonationTable = ({ organisationUser }) => {
  ...
  useEffect(() => {
    const fetchDonations = async () => {
      try {
        const response = await fetch(`/api/getdonations/${organisationUser}`);
        if (!response.ok) {
        ...
        const initialTotal = donations.reduce((sum, item) => sum + item.amount, 0);
