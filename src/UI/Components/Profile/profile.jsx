...
export const AboutPage = ({organisationUser}) => {
    const [organisationData, setOrganisationData] = useState(null);

    // Fetch the organisation data on component mount
    useEffect(() => {
        if (!organisationUser) return; // Don't make the fetch if there's no organisation name
   
        const fetchOrganisation = async () => {
            try {
                const response = await fetch(`/api/about?organisationName=${organisationUser}`);
